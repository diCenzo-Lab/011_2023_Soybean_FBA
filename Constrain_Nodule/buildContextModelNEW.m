...
nodulatedPlant.rev=[];
for i=1:length(nodulatedPlant.rxns)
    if nodulatedPlant.lb(i) < 0
        nodulatedPlant.rev(i,1)=1;
    else
        nodulatedPlant.rev(i,1)=0;
    end
end
...
nodulatedPlant.lb(findRxnIDs(nodulatedPlant, 'R_Biomass')) = growthThresh;
A = fastcore(C, nodulatedPlant, 1.01e-6);
nodulatedPlant.lb(findRxnIDs(nodulatedPlant, 'R_Biomass')) = 0;
...
transfer4Rxns = nodulatedPlant.rxns(strmatch('TNR_', nodulatedPlant.rxns));
...
exctRxnsI = nodulatedPlant.rxns(strmatch('EXCT_for', nodulatedPlant.rxns));
exctRxnsIId = nodulatedPlant.rxns(strmatch('EXCT_rev', nodulatedPlant.rxns));
