...
model.lb(ATPMaintenanceBacteroid) = MaintenanceRequirementBacteroid * 0.25 * 0.5;
 ...
 perc=[perc,abs(((ro.f*24/1000) - 0.076)/ 0.076)]
...
figure(1)            
            plot(noo,perc*100,'LineWidth',4)
                         hold on, drawnow 
                        plot(noo,((0.2632)*ones(size(noo))*100),'-.','color','k','LineWidth',4)
...
print('RGRvaryingRS_withCo2','-depsc','-loose');
...
figure(2)            
            plot(noo,cost,'LineWidth',4)
                         hold on, drawnow 
                        plot(noo,((3.3491)*ones(size(noo))),'-.','color','k','LineWidth',4)
            legend('Varying R:S','R:S=0.17','Location','Best');
 ...
 x_width=24 ;y_width=15;
 set(gcf, 'PaperPosition', [0 0 x_width y_width]);
print('CNvaryingRS_withco2','-depsc','-loose');
